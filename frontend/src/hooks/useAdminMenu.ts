...
export const useAdminMenu = () => {
  ...
  const updateItem = useMutation({ mutationFn: ({ id, data }: { id: string; data: CreateMenuItemRequest }) => adminMenuService.updateMenuItem(id, data), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['adminMenuItems'] }); } });
